                ...
                <tr>
                    <td>
                        <div style="display: flex; align-items: center; gap: 0.75rem;">
                            <div style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, #8B3A3A, #722F37); display: flex; align-items: center; justify-content: center; color: white; font-weight: bold;">
                    ...
                    <td>
                        <div style="display: flex; gap: 0.5rem;">
                            @if(!$user->is_admin)
                                <button data-user-id="{{ $user->id }}" data-user-name="{{ $user->name }}" class="btn view-orders-btn" style="padding: 0.25rem 0.75rem; font-size: 0.875rem; background: #3b82f6; color: white;">
                                    الطلبات
                                </button>
                            @endif
                            <button data-user-id="{{ $user->id }}" class="btn btn-secondary edit-user-btn" style="padding: 0.25rem 0.75rem; font-size: 0.875rem;">
                                تعديل
                            </button>
                            @if($user->id !== auth()->id())
                                <form method="POST" action="/admin/users/{{ $user->id }}" style="display: inline;" class="delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn btn-danger delete-btn" style="padding: 0.25rem 0.75rem; font-size: 0.875rem;" data-message="هل أنت متأكد من حذف هذا المستخدم؟">حذف</button>
                                </form>
                            @endif
                        </div>
                    </td>

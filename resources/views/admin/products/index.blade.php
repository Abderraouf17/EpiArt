                ...
                <div class="bg-white border rounded-lg p-4 shadow-sm">
                    <div class="flex items-start gap-4 mb-3">
                        ...
                        <div class="flex-1">
                            ...
                            <span class="text-sm text-gray-500 block mb-1">{{ $product->category->name }}</span>
                            <div class="font-bold text-[#8B3A3A]">{{ number_format($product->price, 0) }} DA</div>
                        </div>
                        @if($product->is_featured)
                            <span class="badge badge-success text-xs">مميز</span>
                        @endif
                    </div>

                    <div class="flex justify-end gap-2 pt-3 border-t mt-2">
                        <button data-product-id="{{ $product->id }}"
                            class="btn btn-secondary edit-product-btn text-sm py-1 px-3">تعديل</button>
                        <form method="POST" action="/admin/products/{{ $product->id }}" class="delete-form inline">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="btn btn-danger delete-btn text-sm py-1 px-3"
                                data-message="هل أنت متأكد من حذف هذا المنتج؟">حذف</button>
                        </form>
                    </div>
                </div>
            @endforeach
                ...
                <tbody>
                    @foreach($products as $product)
                        <tr>
                            ...
                            <td>
                                <button data-product-id="{{ $product->id }}" class="btn btn-secondary edit-product-btn"
                ...
                <form id="productForm" method="POST" action="/admin/products" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div id="existing-images-container"
                            style="display: flex; gap: 0.5rem; margin-bottom: 0.5rem; flex-wrap: wrap;"></div>
                        <input type="file" name="image_files[]" multiple accept="image/*" id="productImages"
                        ...
                        <small style="display: block; margin-top: 0.25rem; color: #6b7280;">صيغ الصور المدعومة: JPEG, PNG,
                            GIF, BMP, SVG, WEBP. بحد أقصى 3 صور.</small>
                        <div id="product-images-preview"
                            style="display: flex; gap: 0.5rem; margin-top: 0.5rem; flex-wrap: wrap;"></div>
                    </div>

                    <div class="form-group">
                        <label style="font-weight: 600; margin-bottom: 0.5rem; display: block;">التنويعات (حتى 5
                            تنويعات)</label>
                        <div id="variantsContainer"
                            style="display: flex; flex-direction: column; gap: 1rem; border: 1px solid #e5e7eb; padding: 1rem; border-radius: 6px; background: #f9fafb;">
                            <!-- Variants will be added here -->
                        </div>
                        <button type="button" onclick="addVariant()" class="btn"
                            style="margin-top: 0.5rem; background: var(--primary); color: white; padding: 0.5rem 1rem; font-size: 0.875rem;">+
                            إضافة تنويع</button>
                        <small style="display: block; margin-top: 0.5rem; color: #6b7280;">يمكنك إضافة حتى 5 تنويعات (حجم،
                            لون، قطع)</small>
                    </div>

                    <div class="form-group">
                        <div
                            style="background: #fef3c7; border: 2px solid #fbbf24; border-radius: 8px; padding: 1rem; margin-top: 1rem;">
                            <label style="display: flex; align-items: start; gap: 0.75rem; cursor: pointer;">
                                <input type="checkbox" name="is_featured" id="productFeatured" value="1"
                                    style="width: 20px; height: 20px; margin-top: 2px; cursor: pointer;">
                                <div>
                                    <span
                                        style="font-weight: 600; color: #92400e; display: block; margin-bottom: 0.25rem;">⭐
                                        منتج مميز</span>
                                    <small style="color: #78350f; display: block; line-height: 1.5;">سيظهر هذا المنتج في قسم
                                        "المنتجات المميزة" على الصفحة الرئيسية للموقع، مما يجذب انتباه العملاء ويزيد من فرص
                                        البيع.</small>
                                </div>
                            </label>
                        </div>

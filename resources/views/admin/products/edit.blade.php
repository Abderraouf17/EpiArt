...
<div class="card">
    <form method="POST" action="/admin/products/{{ $product->id }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label>الفئة</label>
            <select name="category_id" required style="background-position: left 0.5rem center; padding-right: 0.75rem; padding-left: 2.5rem; direction: rtl;">
        ...
        <div class="form-group">
            ...
            <div style="display: flex; gap: 0.5rem; margin-bottom: 1rem; flex-wrap: wrap;" id="all-images-container">
                @foreach($product->images as $image)
                    <div style="position: relative; width: 80px; height: 80px;" class="existing-image" data-image-id="{{ $image->id }}">
                        <img src="{{ $image->image_url }}" alt="{{ $product->name }}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 6px; border: 2px solid #8B3A3A;">
                        <button type="button" class="delete-image-btn" style="position: absolute; top: -5px; right: -5px; width: 20px; height: 20px; border-radius: 50%; background: #dc2626; color: white; border: none; cursor: pointer; font-size: 12px; line-height: 1; padding: 0; display: flex; align-items: center; justify-content: center;">×</button>
                    </div>
                @endforeach
            </div>
            
            <input type="file" name="image_files[]" multiple accept="image/*" id="imageUpload" onchange="previewNewImages(event)" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 6px;" />
                ...
                <div class="variation-item" style="background: #f9fafb; padding: 1rem; border-radius: 6px; margin-bottom: 1rem;">
                    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr auto; gap: 1rem; align-items: end;">
                        <div class="form-group" style="margin: 0;">
                            ...
                            <select name="variations[{{ $index }}][type]" style="background-position: left 0.5rem center; padding-right: 0.75rem; padding-left: 2.5rem; direction: rtl;">
                                <option value="">اختر</option>
                                @foreach($variationTypes as $type)
                                    <option value="{{ $type }}" @if($variation->type == $type) selected @endif>{{ ucfirst($type) }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group" style="margin: 0;">
                        ...
                        <div class="form-group" style="margin: 0;">
                            ...
                            <input type="number" name="variations[{{ $index }}][additional_price]" step="0.01" value="{{ $variation->additional_price }}">
                        </div>
                    </div>
                </div>
            @endforeach

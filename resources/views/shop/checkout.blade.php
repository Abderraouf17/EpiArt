...
@section('content')
    @php
        ...
        // Split user name (only if authenticated)
        $user = auth()->user();
        $firstName = '';
        $lastName = '';
        $userEmail = '';
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if(empty($cart))
                <div class="max-w-md mx-auto text-center py-16 bg-white rounded-xl shadow-sm border border-gray-100">
                    ...
                    <a href="/shop"
                        class="inline-block px-8 py-3 bg-[#8B3A3A] text-white rounded-lg hover:bg-[#722F37] transition font-semibold">تصفح
                        المتجر</a>
                </div>
            @else
                <form method="POST" action="/checkout/process" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <input type="text" name="first_name" required
                                        class="w-full px-4 py-3 rounded-lg border border-gray-200 focus:ring-2 focus:ring-[#8B3A3A] focus:border-[#8B3A3A] outline-none transition bg-gray-50 focus:bg-white"
                            ...
                            <div class="mb-6" x-data="{ 
                                ...
                                <div class="relative">
                                    ...
                                    <button @click="open = !open" type="button"
                                        class="w-full bg-white border border-gray-200 rounded-lg px-4 py-3 flex justify-between items-center text-right focus:ring-2 focus:ring-[#8B3A3A] transition">
                                        <span x-text="selected ? options.find(o => o.value == selected).label : 'اختر الولاية'"
                                    ...
                                    <div x-show="open" @click.away="open = false"
                                        ...
                                        <div class="p-2 border-b border-gray-100">
                                            <input type="text" x-model="search" placeholder="بحث..."
                                                class="w-full px-3 py-2 bg-gray-50 rounded-md border-none focus:ring-0 text-sm">
                                        </div>
                                        <div class="overflow-y-auto flex-1">
                                            ...
                                            <div x-show="filteredOptions.length === 0"
                                                class="px-4 py-3 text-gray-500 text-sm text-center">لا توجد نتائج</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <label class="cursor-pointer relative">
                                        <input type="radio" name="delivery_type" value="home" class="peer sr-only" required
                                            checked>
                                        <div
                                            class="p-4 rounded-xl border-2 border-gray-200 peer-checked:border-[#8B3A3A] peer-checked:bg-[#8B3A3A]/5 transition flex flex-col items-center gap-3 text-center hover:border-gray-300 h-full">
                                            <div
                                                class="w-10 h-10 rounded-full bg-gray-100 peer-checked:bg-[#8B3A3A] flex items-center justify-center transition-colors">
                                                <svg class="w-6 h-6 text-gray-500 peer-checked:text-white" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                                                </svg>
                                            </div>
                                            <span class="text-sm font-bold text-gray-700 peer-checked:text-[#8B3A3A]">توصيل
                                                للمنزل</span>
                                        </div>
                                    </label>
                                    <label class="cursor-pointer relative">
                            ...
                            <div>
                                ...
                                <textarea name="address" rows="3"
                                    class="w-full px-4 py-3 rounded-lg border border-gray-200 focus:ring-2 focus:ring-[#8B3A3A] focus:border-[#8B3A3A] outline-none transition bg-gray-50 focus:bg-white"
                                    placeholder="الحي، الشارع، رقم المنزل...">{{ old('address') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6 sm:p-8 sticky top-6">
                            ...
                            <div class="space-y-4 max-h-96 overflow-y-auto pr-2 mb-6 custom-scrollbar">
                                @foreach($cart as $item)
                                    ...
                                    <div class="flex gap-4">
                                        <div
                                            class="w-16 h-16 rounded-md overflow-hidden bg-gray-100 flex-shrink-0 border border-gray-200">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <div class="flex justify-between items-center mt-1">
                                                ...
                                                <p class="text-sm font-semibold text-[#8B3A3A]">{{ number_format($itemTotal, 2) }}
                                                    DA</p>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <button type="submit"
                                class="w-full mt-8 bg-[#8B3A3A] text-white py-4 rounded-xl font-bold text-lg hover:bg-[#722F37] shadow-lg shadow-[#8B3A3A]/20 transition transform hover:-translate-y-0.5 active:scale-[0.98]">
                                تأكيد الطلب
                            </button>
